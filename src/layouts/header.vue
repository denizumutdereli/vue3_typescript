<template>
  <n-layout-header bordered>
    ...
    <n-breadcrumb>
      ...
      <n-breadcrumb-item>Home Page</n-breadcrumb-item>
    </n-breadcrumb>
    <n-space :size="20" align="center" style="line-height: 1">
      ...
      <n-tooltip>
        <template #trigger>
          <n-a href="https://github.com/denizumutdereli" target="_blank">
      ...
      <n-popover trigger="click" placement="bottom-end" :width="300">
        ...
        <n-tabs type="line" justify-content="space-evenly" style="--pane-padding: 0">
          ...
          <n-tab-pane name="messages" tab="Messages (6)">
            <n-list style="margin: 0">
              <n-list-item v-for="i in 6" :key="i"> Messages {{ i }} </n-list-item>
            </n-list>
          </n-tab-pane>
        </n-tabs>
      </n-popover>
...
const options = computed(() => [
  { key: 'me', label: `Hey, ${me.value?.username as string}!` },
  { key: 'divider', type: 'divider' },
  { key: 'profile', label: () => h(RouterLink, { to: '/profile' }, 'To Profile') },
...
const handleOptionsSelect = async (key: unknown): Promise<void> => {
  if (key as string === 'logout') {
    ...
    message.success(`Welcome back, ${me.value?.username as string}!`)
  }
}
</script>
